...
public class DisciplineTabAdminService : IDisciplineTabAdminService
{
    ...
    /// <summary>True when the student has selected the required number of add disciplines per semester (3ù8) per educational program.</summary>
    ...
    public async Task<(int? bindId, string? error)> CreateBindAsync(AddDisciplineBindDto dto)
    {
        ...
        // »ÒÔ‡‚ÎÂÌÓ ÒÓÁ‰‡ÌËÂ BindAddDiscipline: Ò‚ÓÈÒÚ‚Ó InProcess ÓÊË‰‡ÂÚ BitArray, ‡ ÌÂ int
    ...
    /// <summary>
    /// After the latest completed discipline choice period for the faculty, lists students who still do not satisfy add-discipline normatives (semesters 3ù8) for their program.
    /// </summary>
