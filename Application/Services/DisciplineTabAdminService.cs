...
public class DisciplineTabAdminService : IDisciplineTabAdminService
{
    ...
    private readonly IDisciplineTabRepository _disciplineTabRepository;
    ...
    public DisciplineTabAdminService(
        ...
        IMapper mapper,
        IDisciplineTabRepository disciplineTabRepository)
    {
        ...
        _context = context;
        _mapper = mapper;
        _disciplineTabRepository = disciplineTabRepository;
    }
    ...
    {
        if (program == null || program.SelectiveDisciplineBySemestr == null) return false;
        for (int sem = 0; sem < program.SelectiveDisciplineBySemestr.Count; sem++)
        {
            var required = program.SelectiveDisciplineBySemestr[sem];
    ...
    {
        var (totalCount, items) = await _studentListRepository.GetStudentsBySelectiveDisciplineAsync(query);
        var totalPages = (int)Math.Ceiling(totalCount / (double)query.PageSize);

        return new PaginatedResponseDto<AdminStudentBySelectiveDisciplineDto>
        {
            TotalItems = totalCount,
            TotalPages = totalPages,
    ...
    public async Task<FullDisciplineWithDetailsDto?> GetDisciplineWithDetailsAsync(Guid id) =>
        await _disciplineTabRepository.GetDisciplineWithDetailsDtoAsync(id);
